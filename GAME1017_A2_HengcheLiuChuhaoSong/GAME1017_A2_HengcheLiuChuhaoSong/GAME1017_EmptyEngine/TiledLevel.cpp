...
TiledLevel::TiledLevel(const unsigned short r, const unsigned short c, const int w, const int h,
	const char* tileDate, const char* levelData, const char* tileKey) : m_rows(r), m_cols(c), m_tileKey(tileKey)
{
	ifstream inFile(tileDate);  //create the 16 prototypes Tiles.
	if (inFile.is_open())
	{
		char key;
		int x, y;
