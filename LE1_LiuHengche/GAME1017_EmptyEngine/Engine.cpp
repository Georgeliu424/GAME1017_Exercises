	...
	if (SDL_Init(SDL_INIT_EVERYTHING) == 0) // If initialization is okay...
	{
		...
		if (m_pWindow != nullptr)
		{
			...
			if (Mix_Init(MIX_INIT_MP3) != 0)
			{
				...
				m_pSounds = Mix_LoadWAV("Aud/coin.wav");
				m_pSounds = Mix_LoadWAV("Aud/unlock.wav");
			}
...
{
	static Engine instance;//Going to create one single instance in a global scope, Magic statics.
	return instance;
}
